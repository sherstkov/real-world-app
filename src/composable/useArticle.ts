...
export function useArticles() {
  ...
  async function fetchArticles(): Promise<void> {
    ...
    if (articlesType.value === 'my-feed') {
      ...
      responsePromise = api.articles.getArticles(pageToOffset(page.value)).then((res) => res.data);
    }
...
const routeNameToArticlesType: Partial<Record<Routes, ArticlesType>> = {
  [HOME_ROUTE_NAME]: 'global-feed',
  [PERSONAL_FEED_ROUTE_NAME]: 'my-feed',
  [TAG_ROUTE_NAME]: 'tag-feed',
  [PROFILE_ROUTE_NAME]: 'user-feed',
  [PROFILE_FAVORITES]: 'user-favorites-feed',
};
...
function useArticlesMeta(): UseArticlesMetaReturn {
  ...
  watch(
    () => route.name,
    (routeName) => {
      const possibleArticlesType = routeNameToArticlesType[routeName as Routes];
